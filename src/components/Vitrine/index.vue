...
export default {
  ...
  methods: {
    ...
    handleOnClickCategory() {
      if (innerWidth <= 800) this.isOpen = !this.isOpen;
    },
    ...
    goToProduct(product) {
      ...
      sessionStorage.setItem("otherProducts", JSON.stringify(this.currentBrand.products));
      ...
      if (showAll) {
        ...
        for (let i = 1; i < this.productCategories.length; i++) {
          ...
          fetchFrom(targetUrl, (res) => {
            this.currentBrand.products = this.currentBrand.products.concat(
              this.currentBrand.products,
              res.products
            );
          });
...
<template>
  ...
  <div v-if="isLoaded">
    <Navigator :is-open="isOpen" @click-close="handleOnClickCategory">
      <div
        ...
        @click="handleOnChangeCategory(category)"
      >
        <strong>{{ toPascalCase(category) }}</strong>
      </div>
    </Navigator>
  ...
  <div class="spinner-border text-warning spinner" role="status" v-if="!isLoaded"></div>
</template>
